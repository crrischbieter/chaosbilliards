...
for K in range(M):
    #Reinitialize variables on every loop
    xi = (K/M)
    yi = 0
    alpha0 = 69 * ( np.pi / 180 )
    ...
    #Determine four intersections
    z1 = intersection2(vi,vf,b1,b2)
    z1 = (1,z1[1])
    z2 = intersection1(vi,vf,b4,b5)
    z2 = (z2[0],1)
    z3 = intersection2(vi,vf,b5,b6)
    z3 = (0.75,z3[1])
    z4 = intersection1(vi,vf,b6,b1)
    z4 = (z4[0],0.75)
    #Determine which of the four intersections actually happens.
    if z1[1] < 0.75:
        vi = z1
    ...
    for I in range(N):
        ...
        z3 = (-1,z3[1])
        z4 = intersection1(vi,vf,b4,b5)
        z4 = (z4[0],1)
        z5 = intersection2(vi,vf,b5,b6)
        z5 = (0.75,z5[1])
        z6 = intersection1(vi,vf,b6,b1)
        z6 = (z6[0],0.75)
        ...
        #The indent has the added fact that one of the intersections is physically impossible, so it must be excluded.
        zlist = []
        #print([z1,z2,z3,z4,z5,z6])
        for J in [z1,z2,z3,z4,z5,z6]:
            if J == vi:
                del J
            #Delete intersection points not on the barrier.
            elif inftynorm(J) > 1: #Delete J if outside box
                del J
            elif inftynorm(J) <= 0.75: #Delete J if inside box
                del J
            elif inftynorm(J) == 1 and J[0] > 0.75 and J[1] == 1: #Delete J if on outside corner
                del J
            elif inftynorm(J) == 1 and J[1] > 0.75 and J[0] == 1: #Delete J if on outside corner
                del J
            elif inftynorm(J) < 1 and J[0] < -0.75:
                del J
            elif inftynorm(J) < 1 and J[1] < -0.75:
                del J
            #Points that intersect with the corner cannot intersect with the barrier 
            #where it doesn't have direct LOS to.
            elif vi[1] == 0.75 and J[1] == 1: #Bottom of indent -/-> top wall
                del J
            elif vi[0] == 0.75 and J[0] == 1: #Side of indent -/-> right wall
                del J
            elif vi[1] == 0.75 and J[0] == 0.75: #Indent -/-> indent
                del J
            elif vi[0] == 0.75 and J[1] == 0.75: #Indent -/-> indent
                del J
            elif vi[1] == 1 and J[1] == 0.75: #Top wall -/-> bottom of indent
                del J
            elif vi[0] == 1 and J[0] == 0.75: #Right wall -/-> side of indent
                del J
            else:
                zlist.append(J)
        #Since now all unphysical/backwards intersections have been eliminated, determine the next point by
        #the closest intersection point from the current point.
        for J in zlist:
            if eunorm((J[0]-vi[0],J[1]-vi[1])) == min([eunorm((i[0]-vi[0],i[1]-vi[1])) for i in zlist]):
                z = J
        ...
        if vi[0] == 1: #Bounce off right verticle wall.
            ...
            if vi[0] > vii[0]: #From left
                ...
                xf = (10 * np.cos(alpha0) + vi[0])
                yf = (-10 * np.sin(alpha0) + vi[1])
        elif vi[1] == 0.75: #Bounce off top wall
            if vi[0] > vii[0]: #From left
                xf = 10 * np.cos(alpha0) + vi[0]
                yf = -10 * np.sin(alpha0) + vi[1]
            elif vi[0] < vii[0]: #From right
                xf = (10 * np.cos(alpha0) + vi[0])
                yf = 10 * np.sin(alpha0) + vi[1]
        elif vi[0] == 0.75: #Bounce off vertical of indent.
            if vi[1] > vii[1]: #From below
                xf = -10 * np.cos(alpha0) + vi[0]
                yf = 10 * np.sin(alpha0) + vi[1]
            if vi[1] < vii[1]: #From above
...
boxx = [-1,0.75,0.75,1,1,-1,-1]
boxy = [1,1,0.75,0.75,-1,-1,1]
